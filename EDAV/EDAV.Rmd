...
Since the numerical data is taken from the official site. We did not observe any missing values or irregular data. The quality of the data is very high. However, during our analysis, we discovered a rounding pattern. We also discovered a pattern in the twitter data.
<br>
<br>
<br>
...
```{r fig.height = 18, fig.width = 6, fig.align = "center" ,results='hide', message=FALSE, warning=FALSE}
...
```{r fig.height = 15, fig.width = 15, fig.align = "center",results='hide', message=FALSE, warning=FALSE}
...
Clutch time, which means the scores of two teams are closed at the end of one game, is thrilling and exciting. However, the number of clutch games is not determined and it may be the most important time in a game, because in the clutch time both team have chance to win and the team who has better performance can take the win, the team with worse performance will waste a night. From this simple plot, we can observe that the WAS played the largest number of cluth time, actually almost 2/3 games in the season for WAS have clutch time, on the other hand, GSW played the least. Also, usually we think a better team is more united and can win the game in the clutch time, and the plot proves that, from the plot we can see the world champion in last year GSW has a very high rate of win in clutch time, that means in some games that two teams have fair performance, GSW can usually take the win back to the bay area; however, the BKN which ranked last one in the league last season, lose almost all clutch games.  
...
This plot gives us a very direct visual presentation on the decomposition of shooting percent of points by different teams. In the clutch time, teams usually use the most familiar tactics they have, so in the plot, we can easily analyse the strategies of each team, so for defensive coaches, maybe they can provide corrsponding the right defensive strategies. For TOR, it has the best two points shoot percent, it can be explained by that the best player of this team Demar DeRozan is one of the best player in mid-range jump shoot, however it had one of the worst 3 points shoot percent in the league, maybe also because Demar DeRozan did not shoot 3 points usually, so for the opponents of TOR, they can leave some distance for TOR's players to shoot three but give much tenser defense for 2 points shoot in the clutch time. On the other hand, HOU as a team devoted for three points shoot, its 3 points percent is pretty high but it has lower two points shoot percent, so for HOU, we should give more denfense to 3 points shoot but leave more space for mid-range shoot.
...
Also, for teams have both bad performance in offense and defensive like PHI, MIA, BKN, PHX, they are all young team and the best players in those team are usually under 22 years old, we can safely say that rookies can not take as much pressure as veterans can take.
<br>
<br>
<br>
...
<br>
<br>
```{r fig.height = 5, fig.width = 8, fig.align = "center",results='hide', message=FALSE, warning=FALSE}
...
```{r fig.height = 5, fig.width = 5, fig.align = "center", results='hide', message=FALSE, warning=FALSE}
##==================================================================
#Plot on All team
...
```{r fig.height = 5, fig.width = 14, fig.align = "center", results='hide', message=FALSE, warning=FALSE}
...
```{r fig.height = 12, fig.width = 14, fig.align = "center", results='hide', message=FALSE, warning=FALSE}
...
### pair plots
```{r fig.height = 20, fig.width = 20, fig.align = "center",results='hide', message=FALSE, warning=FALSE}
